...
const extendAddressLookupTable = async (
  ...
  console.log("sending extensionTransactionV0", extensionTransactionV0);
  const extensionSignature = await sendTransaction(
    extensionTransactionV0,
    connection
  );
  ...
  const status = (await connection.confirmTransaction(extensionSignature))
    .value;
  if (status.err) {
    throw new Error(
      `Transaction ${extensionSignature} failed (${JSON.stringify(status)})`
    );
  }
  console.log(
    "Sent transaction for lookup table extension:",
    extensionSignature
  );
